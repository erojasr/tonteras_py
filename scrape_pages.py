...
while True:
    ...
    #data_marky = {'links':}
    #links.append(soup.select('.storylink')) #.getText())
...
#print (dataframe)
#dataframe = pd.DataFrame([list(itertools.chain(*i)) for i in zip(titles, links, votes)], columns=['Title','Link','Votes'])
dataframe = pd.DataFrame(list(zip(titles,links,votes)), columns=['Title','Link','Votes'])
dataframe['Votes'] = pd.to_numeric(dataframe['Votes'])
